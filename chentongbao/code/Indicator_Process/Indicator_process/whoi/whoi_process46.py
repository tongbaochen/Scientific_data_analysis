...
'''
    ...
    for row in range(2, rowNumber):
        ...
        propertyJson["Year"] = item[2].strip()
            ...
            if column < len(item):
                if item[column].strip() == 'No data':  # 将缺省的数据填充为''空.
                    ...
                    if item[column].find('[') != -1:
                        ...
                        temp[head_row1[column].replace('.', '_') + head_row[column].replace('.', '_')] = float(
...
files = getAllFile('D:\\DataLab_Project\\data\\open_data\\whoi\\type46')
