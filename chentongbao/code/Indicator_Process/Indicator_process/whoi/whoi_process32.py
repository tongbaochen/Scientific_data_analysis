        ...
        for column in range(3, columnNumber):
            ...
            if column < len(item):
                if item[column] == 'No data':#将缺省的数据填充为''空.
                    temp[head_row1[column].replace('.','_') + head_row[column] + item[2]] = ''
                elif item[column] =='':
                    temp[head_row1[column].replace('.','_') + head_row[column] + item[2]] = ''
                    ...
                    if item[column].find('[') != -1:
                        ...
                        temp[head_row1[column].replace('.','_') + head_row[column] + item[2]]  = float((item[column][:(item[column].find('[')-1)]).replace(' ',''))
                    # elif item[column].strip().find('[') == 0:
                    #     temp[head_row[column]] = ''
                    else:
                        temp[head_row1[column].replace('.','_') + head_row[column] + item[2]] = float((item[column]).replace(' ', ''))
...
files = getAllFile('D:\\DataLab_Project\\data\\open_data\\whoi\\type32')
