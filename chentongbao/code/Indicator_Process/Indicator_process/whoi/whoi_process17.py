...
'''
    ...
    #获取第一行
    head_row = mdbuffer[3]
    head_row1 = mdbuffer[1]
    head_row2 = mdbuffer[0]
        ...
        # propertyJson用来设置每个document的json数据属性格式
            ...
            if column < len(item):
                if item[column] == 'No data':#将缺省的数据填充为''空.
                    temp[head_row2[column] + head_row3[column] + head_row[column]] = ''
                else:#将字符串转化为float型数值
                    temp[head_row2[column] + head_row3[column] + head_row[column]] = float(item[column][:item[column].find(' ')])
            # else:
            #     temp[kind] = ''
...
files = getAllFile('D:\\DataLab_Project\\data\\open_data\\whoi\\type17')
