        ...
        for column in range(1, columnNumber):
            ...
            if column < len(item):
                if item[column] == 'No data':#将缺省的数据填充为''空.
                    temp[head_row2[column].replace('.','_')+head_row[column]] = ''
                elif item[column] =='':
                    temp[head_row2[column].replace('.', '_') + head_row[column]] = ''
                    ...
                    if item[column].find('[') != -1:
                        ...
                        temp[head_row2[column].replace('.', '_') + head_row[column]]  = float((item[column][:(item[column].find('[')-1)]).replace(' ',''))
                    # elif item[column].strip().find('[') == 0:
                    #     temp[head_row[column]] = ''
                    else:
                        temp[head_row2[column].replace('.', '_') + head_row[column]] = float((item[column]).replace(' ', ''))
...
files = getAllFile('D:\\DataLab_Project\\data\\open_data\\whoi\\type33')
