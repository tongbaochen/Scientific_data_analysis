...
'''
    ...
    for row in range(3, rowNumber):
        for column in range(1, columnNumber):
            ...
            propertyJson["Year"] = head_row[1].strip()
            ...
            if column < len(item):
                if item[column] == 'No data':#将缺省的数据填充为''空.
                    temp[head_row2[column] + head_row1[column]] = ''
                else:#将字符串转化为float型数值
                    temp[head_row2[column] + head_row1[column]]  = float(item[column])
                # else:
                #     temp[kind] = ''
...
files = getAllFile('D:\\DataLab_Project\\data\\open_data\\whoi\\type23')
