    ...
    for row in range(2, rowNumber):
        for column in range(1,columnNumber):
            ...
            if column < len(item):
                if item[column] == "":
                    temp[kind] = ''
                elif item[column].find(';') == -1:
                    temp[kind] = float(item[column])
                else:
                    temp[kind] = float(item[column][(item[column].find(';') + 1):])
...
files = getAllFile('D:\\DataLab_Project\\data\\open_data\\whoi\\type3')
