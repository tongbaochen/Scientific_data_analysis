...
'''
    ...
    for row in range(1, rowNumber):
        ...
        #     propertyJson[head_row[column]] = item[column]
        #防止list out of range
        for column in range(2, columnNumber - 1):
            if column < len(item):
                if item[column].strip() == 'No data':  # 将缺省的数据填充为''空.
                    temp[head_row[column].replace('.', '_')] = ''
                    ...
                    temp[head_row[column].replace('.', '_')] = ''
                elif item[column].strip() == '--':  # 将缺省的数据填充为''空.
                    temp[head_row[column].replace('.', '_')] = ''
                elif item[column].strip() == '. . .':  # 将缺省的数据填充为''空.
                    temp[head_row[column].replace('.', '_')] = ''
                elif item[column] == '':
                    ...
                    if item[column].find('[') != -1:
                        ...
                        temp[head_row[column].replace('.', '_')] = float(
                            (item[column][:(item[column].find('[') - 1)]).replace(' ', ''))
                    # elif item[column].strip().find('[') == 0:
                    #     temp[head_row[column]] = ''
                    else:
                        temp[head_row[column].replace('.', '_')] = float((item[column]).replace(' ', ''))
...
files = getAllFile('D:\\DataLab_Project\\data\\open_data\\whoi\\type42')
