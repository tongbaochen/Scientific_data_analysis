        ...
        for column in range(3, columnNumber):
            if column < len(item):
                if item[column].strip() == 'No data':  # 将缺省的数据填充为''空.
                    temp[head_row[column].replace('.', '_') + item[2].strip()] = ''
                    ...
                    if item[column].find('[') != -1:
                        # print((item[column][:(item[column].find('[')-1)]).replace(' ',''))
                         temp[head_row[column].replace('.', '_') + item[2].strip()] = float(
                            (item[column][:(item[column].find('[') - 1)]).replace(' ', ''))
                    # elif item[column].strip().find('[') == 0:
                    #     temp[head_row[column]] = ''
                    else:
                         temp[head_row[column].replace('.', '_') + item[2].strip()] = float((item[column]).replace(' ', ''))
...
files = getAllFile('D:\\DataLab_Project\\data\\open_data\\whoi\\type39')
