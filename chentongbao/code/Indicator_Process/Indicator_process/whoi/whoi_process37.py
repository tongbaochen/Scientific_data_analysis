        ...
        # propertyJson用来设置每个document的json数据属性格式
            ...
            if column < len(item):
                if item[column].strip() == 'No data':  # 将缺省的数据填充为''空.
                    temp[head_row1[column].replace('.', '_') + head_row[column]] = ''
                    ...
                    if item[column].find('[') != -1:
                        ...
                        temp[head_row1[column].replace('.', '_') + head_row[column]] = float(
                            (item[column][:(item[column].find('[') - 1)]).replace(' ', ''))
                    # elif item[column].strip().find('[') == 0:
                    #     temp[head_row[column]] = ''
                    else:
                        temp[head_row1[column].replace('.', '_') + head_row[column]] = float((item[column]).replace(' ', ''))
...
files = getAllFile('D:\\DataLab_Project\\data\\open_data\\whoi\\type37')
