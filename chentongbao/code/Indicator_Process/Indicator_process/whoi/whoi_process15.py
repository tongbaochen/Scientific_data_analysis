        ...
        for column in range(3, columnNumber):
            if column < len(item):
                if item[column] == 'No data':#将缺省的数据填充为''空.
                    temp[head_row1[column] + head_row[column]] = ''
                else:
                    temp[head_row1[column] + head_row[column]] = float(item[column][:item[column].find(' ')])
        # else:
        #     temp[kind] = ''
...
files = getAllFile('D:\\DataLab_Project\\data\\open_data\\whoi\\type15')
