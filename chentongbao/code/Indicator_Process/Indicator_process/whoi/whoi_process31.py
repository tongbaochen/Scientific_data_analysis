...
'''
    ...
    head_row2 = mdbuffer[1]
    head_row3 = mdbuffer[2]


    #获得列数
    columnNumber = len(mdbuffer[3])
    ...
    time_set = set(time)
    '''
    for row in range(4, rowNumber):
        for column in range(1, columnNumber):
            ...
            propertyJson["Year"] = head_row1[column].strip()
            ...
            #防止list out of range
            if column < len(item):
                if item[column] == 'No data':#将缺省的数据填充为''空.
                    temp[head_row2[column].replace('.','_')+head_row[column]+head_row3[column]] = ''
                elif item[column] =='':
                    temp[head_row2[column].replace('.','_') + head_row[column] + head_row3[column]] = ''
                    ...
                    if item[column].find('[') != -1:
                        ...
                        temp[head_row2[column].replace('.','_') + head_row[column] + head_row3[column]]  = float((item[column][:(item[column].find('[')-1)]).replace(' ',''))
                    # elif item[column].strip().find('[') == 0:
                    #     temp[head_row[column]] = ''
                    else:
                        temp[head_row2[column].replace('.','_') + head_row[column] + head_row3[column]] = float((item[column]).replace(' ', ''))
...
files = getAllFile('D:\\DataLab_Project\\data\\open_data\\whoi\\type31')
