...
'''
    ...
    time_set = set(time)
    '''
    # timescal = ['2','2000','2001','2002','2003','2004','2005','2006','2007','2008','2009',
    ...
    for row in range(2, rowNumber):
        ...
        # propertyJson用来设置每个document的json数据属性格式
            ...
            propertyJson["Data_source"] = 'ITU'
            if column < 18:
                if item[column] == "":
                    temp[head1] = ''
                else:
                    temp[head1] = float(item[column].replace("'",''))
            else:
                if item[column] == "":
                    temp[head2] = ''
                else:
                    temp[head2] = float(item[column].replace("'", ''))
...
files = getAllFile('D:\\DataLab_Project\\data\\open_data\\ITU\\itu_data_change\\change2')
